...
unsigned long ecurve_expbits;
unsigned long ecurve_points;
unsigned long ecurve_steps;
unsigned long ecurve_fastorders;
unsigned long ecurve_retries;
...
/*
    ...
    Not computing z saves 1 field multiplication when composing an affine point with JC point (usually called mixed addition).  Squaring (doubling) costs an extra multiplication
...
static inline void ecurve_AJC (ecp_jc_t *p, ecp_jc_t *p1, ff_t x0, ff_t y0, ff_t f1)
{
    ...
    _ff_square(t0,e); _ff_mult(t1,t0,e); _ff_mult(t2,t0,a);                         // t0=e^2, t1=e^3, t2=ae^2
...
void ecurve_AJC_exp_ui (ecp_jc_t *p, ff_t x0, ff_t y0, unsigned long n, ff_t f1)
{
    ...
    i = ui_highbit(pbits)-2;                        // we know the top two bits of the NAF are 10
    _ff_neg(negy0,y0);
    m = (1UL<<i);
    for ( ; m ; m >>= 1 ) {
...
void ecurve_AJC_powers (ecp_jc_t p[], ff_t x0, ff_t y0, int k, ff_t f1)
{
    ...
    ecurve_2AJC (p+1, x0, y0, f1);                            // 7M + 10A
    for ( e=p+k,p+=2 ; p <= e ; p++ ) ecurve_2JC(p,p-1,f1);   // 11M + 8A
ecurve_expbits += k;
}
...
/*
    Compute #E(Fp) for elliptic curves y^2=f(x) with 2 < p < 2^40, optimized for p around 2^30.
    Assumes f monic and, for p >3, of the form x^3+f1*x+f0 with nonzero discriminant.
    ...
    This information can be used to speed up later group structure computations.
*/
long ecurve_order (long *pd, ff_t f[4])
{
    ...
    */
    ...
    a = ( tor3==1 && _ff_p1mod3 ? -1 : 0 );                     // if neither the curve or its twist has 3-torsion and p=1 mod 3, we must have a_p=0 and group order 2 mod 3
    ...
    sts = 0;
    int emin = ceil(sqrt(min));
    for ( i = 0 ; i < ECURVE_ORDER_RETRIES || e < emin ; i++ ) {
        if ( low==high ) {exp=low; break; }
        if ( ! ecurve_random_point(&x,&y,h) ) continue;             // this can fail, e.g.  y^2=x^3+2x+2 over F_3 has no affine rational points
//printf ("%lu: e=%ld, Random point (%ld,%ld) on curve ", _ff_p, e, _ff_get_ui(x), _ff_get_ui(y));  ff_poly_print(h,3);
        ...
        low = _ui_ceil_ratio(min,E);
        high = max/E;
        // there are a lot of optimizations we could insert here, but none of them apply often enough to be worth doing, so keep it simple.
ecurve_retries++;
    }
    ...
    /*
        ...
        In the two 3-torsion cases, we will have chosen h to be the twist with 3-torsion and should have succeeded above.  If not we must not have computed
    ...
    */
    ...
    if ( M > max ) return ecurve_order(pd,f); // this can happen if e is wrong
    // if ( M > max ) { printf ("Error: no multiple M of ambiguous e=%ld satisfies M/e | gcd(e,p-1) (p=%ld, A=%ld, B=%ld).\n", e, _ff_p, _ff_get_ui(f[1]), _ff_get_ui(f[0])); abort(); }
...
int ecurve_test_maximal (ff_t f[4], int flag)
{
    ecp_jc_t t[1];
    ff_t x,y;
    
    while ( ! ecurve_random_point(&x,&y,f) );
    if ( flag >= 0 ) {
        ecurve_AJC_exp_ui(t,x,y,_ff_p+1+floor(2*sqrt(_ff_p)),f[1]);
        if ( ecurve_JC_id(t) ) return 1;
    }
    if ( flag <= 0 ) {
        ecurve_AJC_exp_ui(t,x,y,_ff_p+1-floor(2*sqrt(_ff_p)),f[1]);
        if ( ecurve_JC_id(t) ) return 1;
...
#else
#define BSGS_MAX_STEPS      16384               // will handle BSGS for p<2^54, assuming 2 and 3 torsion are known
#endif
...
printf("affine giant step (%ld,%ld)\n", _ff_get_ui(gstep_x),_ff_get_ui(gstep_y));
    ...
    if ( j < dsteps ) {                                                                 // if a giant step hit the identity, we're done
        o = gbase-j*gspace;  if ( pflag ) { *exp=o; return 1; }                         // it might actually be faster to continue, but its simpler to catch this now and its rare in any case
        o = ecurve_fastorder (b_x, b_y, o, f1);  return set_exp(exp,o,low,high);
    }
