...
unsigned long _ff_p3_m;                     // p=3^p3_e*p3_m+1 when p=1mod3, p=3^p3_e*p3_m-1 when p=2mod3
...
void ff_setup_ui (unsigned long p)
{
    ...
    _ff_fifth=_ff_seventh=_ff_eleventh=_ff_thirteenth=0;    // ditto
    ff_poly_stack_init (0,0);
    ff_ext_setup();
    return;
}
...
void ff_montgomery_setup (int ring)
{
    ...
    _ff_mont_pni = -t;
    _ff_mont_R = (1UL<<(FF_MONTGOMERY_BITS-1))%p;
    _ff_mont_R += _ff_mont_R;
    if ( _ff_mont_R > p ) _ff_mont_R -= p;
        
    // to avoid depending on multi-precision arithmetic, we square R mod p by repeated doubling
    // probably no slower than a modular reduction anyway
    _ff_mont_R2 = _ff_mont_R;
    for ( i = 0 ; i < FF_MONTGOMERY_BITS ; i++ ) {
    ...
    if ( ring ) return;
    t = 1;
    for ( i = 0 ; i <= 3*FF_MONTGOMERY_BITS ; i++ ) {
        _ff_mont_itab[i] = t;
        t += t;                                                 // note that _ff_p < 2^(FF_MONTGOMERY_BITS-1) so overflow can't occur
        if ( t >= _ff_p ) t -= _ff_p;
    }
  //printf ("Montgomery setup p = %lu, -p^{-1} mod R = %lu, R mod p = %lu, R^2 mod p = %lu\n", _ff_p, _ff_mont_pni, _ff_mont_R, _ff_mont_R2);
}
...
//      if ( k > 2*_ff_pbits ) { printf ("ff_invert: k = %d > 2*pbits!  raw input %lu\n", k, x);  abort(); }
    ...
    v = ff_montgomery1_mult (v, _ff_mont_itab[3*FF_MONTGOMERY_BITS-k]);        // Montgomery lookup table holds powers 2^k to save time
#if ! FF_FAST
...
int ff_parallel_invert_check (ff_t z[], ff_t x[], unsigned n)
{
    ...
    if ( n > FF_MAX_PARALLEL_INVERTS ) {
        for ( i = 0 ; i+FF_MAX_PARALLEL_INVERTS < n ; i += FF_MAX_PARALLEL_INVERTS ) if ( !ff_parallel_invert_check(z+i,x+i,FF_MAX_PARALLEL_INVERTS) ) return 0;
        z += i;  x += i;  n-= i;
    }
    if ( ! n ) return 1;
...
void ff_setup_2g (void)
{
    ...
    if ( _ff_p2_e == 1 ) { _ff_set(_ff_2g,_ff_negone);  _ff_set(_ff_2gi,_ff_negone); _ff_set(_ff_2Sylow_tab[0][0],_ff_negone); _ff_set(_ff_2Sylow_tab[0][1],_ff_negone);  return; }
...
//  if ( _ff_one(e) ) { _ff_set (o[0], a[0]);  return; }
    ...
    i = ui_highbit(e); // index of top bit
    if ( i&1 ) i--;
    m = 3UL<<i;
    _ff_set (b[1], a[0]);
    _ff_square (b[2],b[1]);
...
*/
int _ff_fast_fourth_root (ff_t a[1], ff_t x[1]) // _ff_p must be 3 mod 4, not verified!
{
...
*/
int _ff_fast_sixth_root (ff_t a[1], ff_t x[1])  // _ff_p must be 11 mod 12, not verified!
{
    static mpz_t E;
    static unsigned long lastp;
    static int init;
...
*/
int _ff_fast_eighth_root (ff_t a[1], ff_t x[1])
{
    static mpz_t E;
    static unsigned long lastp;
    static int init;
...
*/
int _ff_fast_sixteenth_root (ff_t a[1], ff_t x[1])
{
    static mpz_t E;
    static unsigned long lastp;
    static int init;
...
// computes determinant of n x n matrix A, destroying A in the process, result is stored in A[0]
...
void ff_matrix_destructive_determinant (ff_t A[], int n)
{
    ...
    switch (n) {
    case 1: return;
    case 2: _ff_diff_2_mults(A[0],A[0],A[1],A[2],A[3]); return;
    case 3: { ff_t t1,t2,t3;
            _ff_diff_2_mults (t1,A[4],A[5],A[7],A[8]); _ff_diff_2_mults(t2,A[2],A[1],A[8],A[7]); _ff_diff_2_mults(t3,A[1],A[2],A[4],A[5]);
            _ff_sum_3_mults(A[0],t1,t2,t3,A[6],A[3],A[0]); } return;
    }
    
    _ff_set_one(x);
    for ( k = 0 ; k < n-1 ; k++ ) {
        if ( _ff_zero(A[k*n+k]) ) {            // if next diagonal entry is zero, look for a row-swap
            for ( i = k+1 ; i < n ; i++ ) if ( _ff_nonzero(A[i*n+k]) ) break;
            if ( i == n ) { _ff_set_zero(A[0]); return; }
            for ( j = k ; j < n ; j++ ) _ff_swap(A[i*n+j],A[k*n+j]);
            ff_negate(x);
        }
        for ( i = k+1 ; i < n ; i++ ) {
            for ( j = k+1 ; j < n ; j++ ) _ff_diff_2_mults(A[i*n+j],A[k*n+k],A[k*n+j],A[i*n+k],A[i*n+j]);
            if ( i > k+1 ) _ff_mult(x,x,A[k*n+k]);
        }
    }
    _ff_invert(x,x);
