...
static inline uint64_t *bm_alloc(long n) { n = bm_w64(n)+1; uint64_t *bm = calloc(n,sizeof(uint64_t)); return bm; }
static inline void bm_free(uint64_t *bm) { free(bm); }
static inline void bm_clear(uint64_t *bm, long n) { memset (bm, 0, 8*(n>>6)); if ( (n&0x3F) ) bm[n>>6] &= ~(bitmap_mask_tab[n&0x3f]-1); }
static inline void bm_setall(uint64_t *bm, long n) { memset (bm, 0xFF, 8*(n>>6)); if ( (n&0x3F) ) bm[n>>6] |= bitmap_mask_tab[n&0x3f]-1; }
