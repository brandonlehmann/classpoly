...
int ecurve_3tor (ff_t f[4])
{
	...
	if ( ! _ff_p1mod3 ) {
		...
		ff_poly_depressed_cubic_resolvent(&t0,g,div);
...
int ecurve_mod3 (ff_t f[4], int *m)
{
	...
	if ( _ff_p1mod3 ) {
		...
		if ( _ff_zero(div[1]) ) {													// the biquadratic case is handled quickly by ff_poly_roots_d4, so let it
			...
			ff_poly_depressed_cubic_resolvent(&t0,a,div);
...
int ecurve_mod4 (ff_t f[4], int odd)
{
	...
	if ( ! odd ) {
		...
		if ( k==1 ) {
			...
			_ff_square(t0,r[0]); _ff_add(t1,t0,t0); _ff_addto(t1,t0); _ff_addto (t1,f[1]);		// compute f'(r) = 3r^2 + f1
