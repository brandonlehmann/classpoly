...
// Sets p[i] = 2^i*(x0,y0) for i in [0,k], with p[i] in reduced Jacobian Chudnovsky coords.
void ecurve_AJC_powers (ecp_jc_t p[], ff_t x0, ff_t y0, int k, ff_t f1);

// computes p[i] = 2*p[i-1] for 1 <= k <= n, with p[i] in reduced Jacobian Chudnovsky coords
...
int ecurve_mod4 (ff_t f[4], int odd);			   			// returns the order of the elliptic curve y^2=f(x) modulo 4, if odd is set it means the caller knows the order is odd
...
static inline void ecurve_from_jinv (ff_t f[4], ff_t j[1])
{
	...
	_ff_set_one(f[3]);
	_ff_set_zero(f[2]);
	if ( _ff_zero(j[0]) ) { _ff_set_zero(f[1]); _ff_set_one(f[0]); return; }      // E_0: y^2=x^3+x+1
	_ff_set_ui(t0, 1728);
	if ( _ff_equal(j[0],t0) ) { _ff_set_one(f[1]); _ff_set_zero(f[0]); return; }  // E_1728: y^2=x^3+x
	// E_j: y^2=x^3+ax+b with a=3j(1728-j) and b=2j(1728-j)^2
	_ff_sub(t1,t0,j[0]);
	_ff_mult(t2,t1,j[0]);
	_ff_mult(t3,t1,t2);
	_ff_add(f[0],t3,t3); _ff_add(f[1],t2,t2); _ff_addto(f[1],t2);
}

// computes the absolute value of the trace of Frobenius for an elliptic E/Fp with j(E)=j (uses y^2=x^3+1 and y^2=x^3+x for j=0,1728)
static inline int ecurve_jtrace (ff_t j)
    { ff_t f[4]; ecurve_from_jinv(f,&j); int t = (long)_ff_p+1L-ecurve_order(0,f); return t<0?-t:t; }
...
static inline int ecurve_random_point (ff_t px[1], ff_t py[1], ff_t f[2])
{
	...
	_ff_set(px[0],x);
	_ff_set(py[0],z);
extern unsigned long ecurve_points;
ecurve_points++;
	return 1;
}
