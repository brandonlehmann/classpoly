...
// Requires f monic and of degree at most 4 (returns 0 if either failes to be true)
// requires f monic
void ff_poly_disc_small (ff_t D[1], ff_t f[], int d_f)
{
    assert ( _ff_one(f[d_f]) );
    assert ( d_f >= 0 && d_f <= 4 );
    switch (d_f) {
    case 0: _ff_set_one(D[0]); return;
    case 1: _ff_set_one(D[0]); return;
    case 2: ff_poly_disc_2(D,f); return;
    case 3: ff_poly_disc_3(D,f); return;
    case 4: ff_poly_disc_4(D,f); return;
    }
}

// requires f monic deg at most 4
int ff_poly_disc_legendre_small (ff_t f[], int d_f)
{
    assert ( _ff_one(f[d_f]) && d_f <= 4 );
    switch (d_f) {
    case 0: return 1;
    case 1: return 1;
    case 2: return ff_poly_disc_legendre_2 (f);
    case 3: return ff_poly_disc_legendre_3 (f);
    case 4: return ff_poly_disc_legendre_4 (f);
    }
    return 0;
}
...
// Replaces f by f mod g, with g monic (not verified), result is zero-filled to degree d_g-1 (so h must have space for this!)
void ff_poly_mod_small (ff_t *h, int *d_h, ff_t *f, int d_f, ff_t *g, int d_g)
{
    ff_t s[d_f+1], t[d_f+1], *fp, *gp;

    if ( d_g <= 0 ) { assert ( d_g >= 0 ); if ( d_h ) *d_h = -1; }
        
    if ( d_f < d_g ) { ff_poly_copy (h, d_h, f, d_f);  memset (h+d_f+1, 0, d_g-1-d_f); return; }
    
    fp = ff_poly_copy (s, 0, f, d_f);
    
    // if g is not monic copy it and make it so
    gp = ( _ff_one(g[d_g]) ) ? g : ff_poly_monic (ff_poly_copy (t, 0, g, d_g), 0, g, d_g);
...
void ff_poly_square_mod_small_big  (ff_t o[], ff_t f[], ff_t g[], int n) 
{
    ...
    if ( n < 0 ) { fprintf(stderr, "Invalid zero modulus in ff_poly_mod_small_big!\n"); abort(); }
    if ( n <= 20 ) ff_poly_square_mod_small (o, f, g, n);
    ...
    for ( int i = n-20 ; i >=0 ; i-- ) { ff_unary_conv(&t0,f+i,n-i+1); ff_conv(&t1,t+i+2, g+i+1, n-i-1); _ff_add(t[i],t0,t1); }
    _ff_set(o[n],t[0]);
    for ( int i = n-1 ; i > 19 ; i-- ) { ff_unary_conv(&t0,f,i+1); ff_conv(&t1,t+1,g,i+1);  _ff_add(o[i],t0,t1); }
...
// computes f^2 mod g where g=x^n - g[n-2]x^{n-2} - ... - g[0] (note signs!!)  and f has degree n-1
void ff_poly_square_mod_small (ff_t o[], ff_t f[], ff_t g[], int n)
{
    switch (n) {
    case 0: break;
    case 1: _ff_set(o[0],f[0]); break;
    case 2: ff_poly_square_mod_2 (o,f,g); break;
...
int _ff_poly_roots_d4_mod3 (ff_t r[4], ff_t f[5])
{
    ff_t y, g[4];
...
//if ( ! _ff_zero(t1) ) { printf ("failed to find root of "); ff_poly_print(f,3); printf ("p=%ld, d=%ld, t=%ld, r=%ld, s=%ld, root=%ld\n", _ff_p, _ff_get_ui(t), _ff_get_ui(r), _ff_get_ui(s), _ff_get_ui(roots[0])); abort(); }
            ...
            // don't want to have to compute a new generator of the Sylow 3-subgroup in F_p^2 every time 
            ...
            // When e==1, life is pretty simple: we know v is a cube, and there is only one cube in the Sylow 3-subgroup, namely 1, and its inverse cube root is 1
            ...
            ff2_mult(g,g,w);                                    // g = v^(((p-1)m-k)/3)  (note that (p+2)(m-k)/3 + (k*3^(e-1)-1)m = (pm+2m-kp-2k+k3^em-3m)/3 = (pm-m-kp-2k+k(p+1))/3=((p-1)m-k)/3
...
int _ff_poly_roots_d4 (ff_t r[4], ff_t f[5], ff_t *pd, int flags)
{
    ...
    ff_poly_depressed_cubic_resolvent(&t,g,f);
...
// computes g(x) = h1(h2(x)) mod f(x) with aliasing permittted, assumes f = x^2 - f0 (in fact f is irrelevant because we are composing linear polys)
// 2M
void ff_poly_compose_mod_2 (ff_t g[2], ff_t h1[2], ff_t h2[2], ff_t f[1])
{
    ff_t t;
    
    _ff_sum_2_mults (t,h1[1],h1[1],h2[0],h2[1]); _ff_set(g[1],t); _ff_set(g[0],h1[0]);
}

// computes g(x) = h1(h2(x)) mod f(x) with overlap permittted, assumes f = x^3 - f1x - f0
// 16M + 10A
void ff_poly_compose_mod_3 (ff_t g[3], ff_t h1[3], ff_t h2[3], ff_t f[2])
{
    ff_t w[3],t0;
    
    _ff_mult(w[2],h1[2],h2[2]); _ff_mult(w[1],h1[2],h2[1]); _ff_mult(w[0],h1[2],h2[0]); _ff_addto(w[0],h1[1]);      // w = h1[2]*h2 + h1[1]
    _ff_set(t0,h1[0]); ff_poly_mult_mod_3 (g,w,h2,f);  _ff_addto(g[0],t0);                                          // w = h2*(h1[2]*h2+h1[1]) + h1[0]
}
...
void ff_poly_compose_mod_4 (ff_t g[4], ff_t h1[4], ff_t h2[4], ff_t f[3])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[3], h2, 3);  _ff_addto(w[0],h1[2]);                                               // w = h1[3]*h2 + h1[2]
    ff_poly_mult_mod_4 (w,w,h2,f);   _ff_addto(w[0],h1[1]);                                                         // w = h2*(h1[3]*h2+h1[2])) + h1[1]
   _ff_set(t0,h1[0]); ff_poly_mult_mod_4 (g,w,h2,f); _ff_addto(g[0],t0);                                            // g = h2*(h2*(h1[3]*h2+h1[2])+h1[1]) + h1[0]
}
...
void ff_poly_compose_mod_5 (ff_t g[5], ff_t h1[5], ff_t h2[5], ff_t f[4])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[4], h2, 4);  _ff_addto(w[0],h1[3]);
    for ( int i = 2 ; i > 0 ; i-- ) {  ff_poly_mult_mod_5 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_5 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_6 (ff_t g[6], ff_t h1[6], ff_t h2[6], ff_t f[5])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[5], h2, 5);  _ff_addto(w[0],h1[4]);
...
void ff_poly_compose_mod_7 (ff_t g[7], ff_t h1[7], ff_t h2[7], ff_t f[6])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[6], h2, 6);  _ff_addto(w[0],h1[5]);
    for ( int i = 4 ; i > 0 ; i-- ) {  ff_poly_mult_mod_7 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_7 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_8 (ff_t g[8], ff_t h1[8], ff_t h2[8], ff_t f[7])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[7], h2, 7);  _ff_addto(w[0],h1[6]);
    for ( int i = 5 ; i > 0 ; i-- ) {  ff_poly_mult_mod_8 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_8 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_9 (ff_t g[9], ff_t h1[9], ff_t h2[9], ff_t f[8])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[8], h2, 8);  _ff_addto(w[0],h1[7]);
    for ( int i = 6 ; i > 0 ; i-- ) {  ff_poly_mult_mod_9 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_9 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_10 (ff_t g[10], ff_t h1[10], ff_t h2[10], ff_t f[9])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[9], h2, 9);  _ff_addto(w[0],h1[8]);
    for ( int i = 7 ; i > 0 ; i-- ) {  ff_poly_mult_mod_10 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_10 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_11 (ff_t g[11], ff_t h1[11], ff_t h2[11], ff_t f[10])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[10], h2, 10);  _ff_addto(w[0],h1[9]);
    for ( int i = 8 ; i > 0 ; i-- ) {  ff_poly_mult_mod_11 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_11 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_12 (ff_t g[12], ff_t h1[12], ff_t h2[12], ff_t f[11])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[11], h2, 11);  _ff_addto(w[0],h1[10]);
    for ( int i = 9 ; i > 0 ; i-- ) {  ff_poly_mult_mod_12 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_12 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_13 (ff_t g[13], ff_t h1[13], ff_t h2[13], ff_t f[12])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[12], h2, 12);
    _ff_addto(w[0],h1[11]);
    for ( int i = 10 ; i > 0 ; i-- ) {  ff_poly_mult_mod_13 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_13 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_14 (ff_t g[14], ff_t h1[14], ff_t h2[14], ff_t f[13])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[13], h2, 13);
    _ff_addto(w[0],h1[12]);
    for ( int i = 11 ; i > 0 ; i-- ) {  ff_poly_mult_mod_14 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_14 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_15 (ff_t g[15], ff_t h1[15], ff_t h2[15], ff_t f[14])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[14], h2, 14);
    _ff_addto(w[0],h1[13]);
    for ( int i = 12 ; i > 0 ; i-- ) {  ff_poly_mult_mod_15 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_15 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_16 (ff_t g[16], ff_t h1[16], ff_t h2[16], ff_t f[15])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[15], h2, 15);
    _ff_addto(w[0],h1[14]);
    for ( int i = 13 ; i > 0 ; i-- ) {  ff_poly_mult_mod_16 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_16 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_17 (ff_t g[17], ff_t h1[17], ff_t h2[17], ff_t f[16])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[16], h2, 16);
    _ff_addto(w[0],h1[15]);
    for ( int i = 14 ; i > 0 ; i-- ) {  ff_poly_mult_mod_17 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_17 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_18 (ff_t g[18], ff_t h1[18], ff_t h2[18], ff_t f[17])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[17], h2, 17);
    _ff_addto(w[0],h1[16]);
    for ( int i = 15 ; i > 0 ; i-- ) {  ff_poly_mult_mod_18 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_18 (g,w,h2,f); _ff_addto(g[0],t0);
}
...
void ff_poly_compose_mod_19 (ff_t g[19], ff_t h1[19], ff_t h2[19], ff_t f[18])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[18], h2, 18);
    _ff_addto(w[0],h1[17]);
...
void ff_poly_compose_mod_20 (ff_t g[20], ff_t h1[20], ff_t h2[20], ff_t f[19])
{
    ...
    ff_poly_scalar_mult (w, 0, h1[19], h2, 19);
    _ff_addto(w[0],h1[18]);
    for ( int i = 17 ; i > 0 ; i-- ) {  ff_poly_mult_mod_20 (w,w,h2,f);  _ff_addto(w[0],h1[i]); }
    _ff_set(t0,h1[0]); ff_poly_mult_mod_20 (g,w,h2,f); _ff_addto(g[0],t0);
}

// computes g(x) = h1(h2(x)) mod f(x) with overlap permittted
// optimized for the case where f is monic and d_h1=d_h2=d_f-1 (zero pad if necessary to achieve this)
// returned result will be zero padded to d_f-1
void ff_poly_compose_mod_small (ff_t g[], int *pd_g, ff_t h1[], int d_h1, ff_t h2[], int d_h2, ff_t f[], int d_f)
{
    ff_t ft[d_f+1], h2t[d_f], w[2*d_f], *fp, *h2p;
    register int i,j,d;
    
    assert (d_f > 0);

    if ( d_f <= 20 && _ff_one(f[d_f]) && _ff_zero(f[d_f-1]) && d_h1 == d_f-1 && d_h2 == d_f-1 ) {
        // this code is more than twice as fast as the generic code below
        fp = ff_poly_copy(ft,0,f,d_f);
        for ( i = 0 ; i < d_f-1 ; i++ ) ff_negate(fp[i]);
        switch (d_f) {
        case 2: ff_poly_compose_mod_2 (g, h1, h2, fp); break;
        case 3: ff_poly_compose_mod_3 (g, h1, h2, fp); break;
        case 4: ff_poly_compose_mod_4 (g, h1, h2, fp); break;
        case 5: ff_poly_compose_mod_5 (g, h1, h2, fp); break;
        case 6: ff_poly_compose_mod_6 (g, h1, h2, fp); break;
        case 7: ff_poly_compose_mod_7 (g, h1, h2, fp); break;
        case 8: ff_poly_compose_mod_8 (g, h1, h2, fp); break;
        case 9: ff_poly_compose_mod_9 (g, h1, h2, fp); break;
        case 10: ff_poly_compose_mod_10 (g, h1, h2, fp); break;
        case 11: ff_poly_compose_mod_11 (g, h1, h2, fp); break;
        case 12: ff_poly_compose_mod_12 (g, h1, h2, fp); break;
        case 13: ff_poly_compose_mod_13 (g, h1, h2, fp); break;
        case 14: ff_poly_compose_mod_14 (g, h1, h2, fp); break;
        case 15: ff_poly_compose_mod_15 (g, h1, h2, fp); break;
        case 16: ff_poly_compose_mod_16 (g, h1, h2, fp); break;
        case 17: ff_poly_compose_mod_17 (g, h1, h2, fp); break;
        case 18: ff_poly_compose_mod_18 (g, h1, h2, fp); break;
        case 19: ff_poly_compose_mod_19 (g, h1, h2, fp); break;
        case 20: ff_poly_compose_mod_20 (g, h1, h2, fp); break;
        }
    ...
    d = d_f-1; // we work in Fp[x]/(f) with polys zero-padded to degree d
    fp = ( _ff_one(f[d_f]) ) ? f : ff_poly_monic (ff_poly_copy (ft,0,f,d_f), 0, f, d_f);
    if ( ! _ff_zero(fp[d_f-1]) && (d_f%_ff_p) ) fp = ff_poly_depress_monic (0, ft, fp, d_f);

    if ( d_h2 != d ) {
        h2p = ff_poly_copy (h2t,0,h2,d_h2);
        if ( d_h2 > d ) ff_poly_mod_small_inplace (h2p, d_h2, fp, d_f);
        else ff_poly_zpad (h2p, d_h2, d);
        d_h2 = d;
    } else {
        h2p = h2;
    }
    
    for ( i = 0 ; i <= d ; i++ ) _ff_mult(w[i], h1[d_h1], h2p[i]);
    _ff_addto(w[0], h1[d_h1-1]);
    for ( i = d_h1-2 ; i >= 0 ; i-- ) {
        // w = w*h2+h1[i]
        for ( j = d ; j >=0 ; j-- ) ff_conv(w+d+j,w+j,h2p+j,d-j+1);
        for ( j = d-1 ; j >= 0 ; j-- ) ff_conv(w+j,w,h2p,j+1);
        _ff_addto (w[0], h1[i]);
        // w = w mod f
        ff_poly_mod_small_inplace (w, 2*d, fp, d_f);
    }
    d = ff_poly_degree (w, d);
    ff_poly_copy (g, pd_g, w, d);
    ff_poly_zpad (g, d, d_f-1);
}
...
void ff_poly_exp_mod_2 (ff_t g[2], ff_t h[2], unsigned long n, ff_t f[2])
{
    ...
    ff_poly_square_mod_2f (x[2],x[1],f);
    ff_poly_mult_mod_2f (x[3],x[2],x[1],f);
    i = ui_highbit(n);
    if ( i&1 ) i--;
    m = 3UL<<i;
    j = (m&n)>>i;
    ...
    for ( m>>=2,i-=2 ; m ; m>>=2, i-=2 ) {
        ff_poly_square_mod_2f (g,g,f);  ff_poly_square_mod_2f (g,g,f);
        j = (m&n)>>i;
...
{
    ff_t f0[d_f+1], g0[d_g+1];

    if ( d_g != d_f+1 ) return 0;
