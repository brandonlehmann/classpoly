...
double *qform_jb_table;
long qform_table_size;
long qform_table_D;
long qform_jb_table_D;
...
void qform_table_free ()
{
    ...
    if ( qform_jb_table ) { free (qform_jb_table); qform_jb_table_D = 0; }
}
...
void qform_sum_jbounds_subgroup (double *psummax, double *pmaxdelta, long p[IQ_MAX_GENS], long n[IQ_MAX_GENS], int k, int d0, int d1, long e, long D, double hf)
{
    ...
    if ( qform_table_D != D ) { err_printf ("qform table not loaded for D=%ld\n", D); exit (0); }
    if ( qform_jb_table_D != D ) {
        if ( qform_jb_table ) free (qform_jb_table);
        qform_jb_table = malloc (qform_table_next*sizeof(*qform_jb_table));
        for ( i = 1 ; i < qform_table_next ; i++ ) qform_jb_table[i] = qform_jbound(qform_table[i].a,D);
        qform_jb_table_D = D;
    }
...
// given n reduced forms of p-power order stored as triples in gen that are known to generate a subgroup of order at most q, compute the order of the group
// if any of these assumptions, the results are undefined (and may include going into an infinite loop, running out of memory, etc...)
long qform_pgroup_order (long *gens, int n, long p, long q, long L)
{
    long a,b,c,u,v,w,x,y,z,h,o,D,j,k;
    int i;

//printf("qform_pgroup_order(%d,%ld,%ld)\n",n,p,q);
    qform_table_init(q,0);
    qform_insert (gens[0],gens[1],gens[2]);
    a=gens[0]; b=gens[1]; c=gens[2];
    qform_square (&u,&v,&w,a,b,c,L);
    while ( ! qform_is_identity(u,v,w) ) { qform_insert(u,v,w); qform_compose (&u,&v,&w,u,v,w,a,b,c,L); }
    h = qform_table_next;
//printf ("First generator [%ld,%ld,%ld] has order %ld\n",a,b,c,h);
    if ( h == q ) return h;
    assert ( h < q );
    D = b*b-4*a*c;
    for ( i = 1 ; i < n ; i++ ) {
        a=gens[3*i]; b=gens[3*i+1]; c=gens[3*i+2];
        if ( qform_lookup (a,b,c) >= 0 ) continue;
        if ( p*h == q ) return p*h;
        for ( o = p, qform_exp (&u,&v,&w,a,b,c,p,L) ; qform_lookup(u,v,w) < 0 ; qform_exp(&u,&v,&w,u,v,w,p,L) ) { o *= p; if ( o*h == q ) return o*h; }
//printf ("Generator [%ld,%ld,%ld] has relative order %ld\n",a,b,c,o);
        assert ( o*h < q );
        // extend subgroup
        u=a; v=b; w=c;
        for ( j = 1 ; j < o ; j++ ) {
            ...
            qform_insert(u,v,w);
            for ( k = 1 ; k < h ; k++ ) {
                qform_compose(&x,&y,&z,u,v,w,qform_table[k].a,qform_table[k].b,qform_c(qform_table[k].a,qform_table[k].b,D),L);
                qform_insert(x,y,z);
            }
            if ( j < o-1 ) { qform_compose(&u,&v,&w,u,v,w,a,b,c,L);  assert (!qform_is_identity(u,v,w)); }
        }
        h *= o;  assert (qform_table_next == h);
    }
    return h;
}
...
// given an estimated multiple of the order m and lower and upper bounds m0, m1, do a center-out BSGS search for a multiple of the order
long qform_order_multiple (long a, long b, long c, long m, long m0, long m1, long L)
{
    long bu,bv,bw,gu,gv,gw,x,y,z;
    register int i,j;

//printf ("qform_order_multiple ([%ld,%ld,%ld], %ld, [%ld,%ld])\n",a,b,c,m,m0,m1);
    if ( qform_is_identity(a,b,c) ) return 1;
    if ( qform_is_2torsion(a,b,c) ) return 2;
    assert ( m > 0 && m >= m0 && m <= m1 );
    qform_exp (&gu,&gv,&gw,a,b,c,m,L);
//printf ("%ld * [%ld,%ld,%ld] = [%ld,%ld,%ld]\n", m, a, b, c, gu, gv, gw);
    if ( qform_is_identity(gu,gv,gw) ) return m;
    qform_table_init (2*sqrt(_max(m1-m,m-m0)), 0);
    bu=a, bv=b, bw=c;
...
//printf ("%d * [%ld,%ld,%ld] = [%ld,%ld,%ld]\n", i, a, b, c, bu, bv, bw);
    for ( i = 2 ;; i++ ) {
        if ( qform_is_identity(bu,bv,bw) ) { qform_table_free(); return i; }
        if ( (j=qform_lookup_hi(gu,gv,gw)) >= 0 ) {
            // we know gm +/- j is a multiple of the order
//printf ("collision between giant step [%ld,%ld,%ld]=%ld*[%ld,%ld,%ld] and baby step [%d,%d,*]=%d*[%ld,%ld,%ld]\n", gu,gv,gw,m,a,b,c,qform_table[j].a,qform_table[j].b,j,a,b,c);
            m += ( qform_table[j].b == gv ? -j : j );
            qform_table_free();
            return m;
        }
        qform_insert_hi (bu,bv,bw);
        qform_compose(&bu,&bv,&bw,bu,bv,bw,a,b,c,L);
//printf ("%d * [%ld,%ld,%ld] = [%ld,%ld,%ld]\n", i+1, a, b, c, bu, bv, bw);
        qform_square (&x,&y,&z,bu,bv,bw,L);
//printf ("%d * [%ld,%ld,%ld] = [%ld,%ld,%ld]\n", 2*i+2, a, b, c, x, y, z);
        qform_compose(&gu,&gv,&gw,gu,gv,gw,x,y,z,L);
        m += 2*i+2;
//printf ("%ld * [%ld,%ld,%ld] = [%ld,%ld,%ld]\n", m, a, b, c, bu, bv, bw);
    }
}

// given an interval [m0,m1] does a BSGS search for multiples of the order of [a,b,c].
// returns -1 if none are found, returns 0 and sets *n=ord([a,b,c]) if more than one are found, otherwise returns 1 and sets n to the unique multiple of ord([a,b,c]) in [m0,m1]
long qform_order_or_multiple (long *n, long a, long b, long c, long m0, long m1, long L)
{
    long bu,bv,bw,gu,gv,gw,x,y,z,m,n1,n2;
    register int i,j,r,s,w;
    
    *n = 0;
    if ( m0 > m1 ) return -1;

//printf ("qform_order_multiple ([%ld,%ld,%ld], %ld, [%ld,%ld])\n",a,b,c,m,m0,m1);
    if ( qform_is_identity(a,b,c) ) { if ( m1 == m0 ) { *n = m0; return 1; } else { *n = 1; return 0; } }
    if ( qform_is_2torsion(a,b,c) ) { if ( m1-m0 < 2 || (m1-m0==2 && (m0&1)) ) { *n = (m0&1) ? m0+1 : m0; return 1; } else { *n = 2; return 0; } }
    if ( m1-m0 < 3 ) {
        qform_exp (&gu,&gv,&gw,a,b,c,m0,L);
        for ( i = 0 ;; i++ ) {
            if ( qform_is_identity(gu,gv,gw) ) { *n = m0+i; return 1; }
            if ( i == m1-m0 ) return -1;
            qform_compose(&gu,&gv,&gw,gu,gv,gw,a,b,c,L);
        }
    }
    r = (long) ceil(sqrt((m1-m0)/2));
    w = 2*r+1;
    for ( s = (m1-m0)/w ; s*w < m1-m0+1 ; s++ );
//printf("r=%d, s=%d, w=%d\n", r, s, w);

    // compute table of baby steps, return if order is found (unlikely)
    qform_table_init (r+1, 0);
    bu=a, bv=b, bw=c;
    qform_insert_hi (bu,bv,bw);
    qform_square (&bu,&bv,&bw,bu,bv,bw,L);
    *n = 0;
    for ( i = 2 ; ; i++ ) {
        if ( qform_is_2torsion(bu,bv,bw) ) { *n = 2*i; goto found_order; }
        if ( (j=qform_lookup_hi(bu,bv,bw)) >= 0 ) { assert (j+1==i); *n = i+j; goto found_order; }
        qform_insert_hi (bu,bv,bw);
        if ( i == r ) break;
        qform_compose(&bu,&bv,&bw,bu,bv,bw,a,b,c,L);
    }

    // compute initial giant step and giant step delta (w=2*r+1)
    n1 = n2 = 0;
    qform_exp (&gu,&gv,&gw,a,b,c,m0+r,L);
    qform_square (&x,&y,&z,bu,bv,bw,L);
    qform_compose (&x,&y,&z,x,y,z,a,b,c,L);
    if ( qform_is_identity(x,y,z) ) { *n = w; goto found_order; }
    for ( i = 0 ; ; i++ ) {
        if ( (j=qform_lookup_hi(gu,gv,gw)) >= 0 ) {
            m = m0+r+i*w + ( qform_table[j].b == gv ? -j : j );
            if ( n1 ) { n2 = m; break; }
            n1 = m;
//printf("n1=%ld n2=%ld\n",n1,n2);
        }
...
//printf ("baby insert %d: %ld,%ld\n", i+1, bu, bv);
    ...
    qform_square(&bu,&bv,&bw,bu,bv,bw,L);
    for ( i = j = 0 ; i < gsteps ; i++ ) {
//printf ("giant lookup %d: %ld,%ld\n", i, gu, gv);
        if ( (j=qform_lookup_hi(gu,gv,gw)) >= 0 ) break;
        qform_compose(&gu,&gv,&gw,gu,gv,gw,bu,bv,bw,L);
    }
    if ( i < gsteps ) {
        if ( !j ) k = 2*i+bsteps; else k = ( qform_table[j].b == gv ? 2*i*bsteps-j : 2*i*bsteps+j );
        if ( k < 0 ) k += n;
    } else {
        k = -1;
    }
...
if ( k >= 0 ) { 
    ...
    if ( ! qform_equal(bu,bv,bw,u,v,w) ) { err_printf ("Discrete log failed <%ld,%ld,%ld>^%ld != <%ld,%ld,%ld>, found match with i=%ld, j=%ld, bsteps=%ld, gsteps=%ld, n=%ld\n", a, b, c, k, u, v, w, i, j, bsteps, gsteps, n); exit (0); }
} else {
    if ( (k=qform_discrete_log_naive(a,b,c,u,v,w,n,L)) >= 0 ) { err_printf ("Discrete log failed to find <%ld,%ld,%ld>^%ld = <%ld,%ld,%ld> with bsteps=%ld, gsteps=%ld, n=%ld\n", a,b,c,k,u,v,w,bsteps,gsteps,n); exit (0); }
}
*/
